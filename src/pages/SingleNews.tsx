...
function SingleNews() {
  ...
  // filter article based on article id which is unique so just one article will be returned and store it at singleNews
  const filteredArticle = news.filter((article: any) => {
    return article.id == articleID;
  });

  const singleNews = filteredArticle[0];
    ...
    <Container maxW={'70vw'}>
      <div className="p-2">
        <img
          ...
          className="w-full"
        />
        <h1 className="text-4xl font-bold my-3">{singleNews?.title}</h1>
        <div className="py-5 mb-7 flex items-center justify-between border-b-2 border-black">
          <div className="flex gap-2 items-center">
            ...
            />
            <span className="font-semibold">@{singleNews?.author.name}</span>
          </div>
        ...
        <div
          dangerouslySetInnerHTML={{
            __html: singleNews?.post ? singleNews?.post : '',
          }}
        ></div>
      </div>
    </Container>
